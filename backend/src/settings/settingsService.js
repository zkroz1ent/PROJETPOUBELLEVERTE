...
exports.getIsWinter = async () => {
    try {
        const setting = await Settings.findOne({ where: { id: 1 } }); // Supposant qu'il y a un enregistrement avec id: 1
        if (!setting) {
            throw new Error('Settings not found.');
        }
        return setting.modeActuelle === 'winter';
    } catch (error) {
        console.error('Erreur lors de la récupération du mode:', error);
        throw error;
    }
